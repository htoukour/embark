...
const requireFromHttpProxyMiddleware = (mod) => (
...
const common = requireFromHttpProxyMiddleware(
  'http-proxy/lib/http-proxy/common',
);
const httpProxyWsIcoming = requireFromHttpProxyMiddleware(
  'http-proxy/lib/http-proxy/passes/ws-incoming',
);

const CRLF = '\r\n';

httpProxyWsIcoming.stream = (req, socket, options, head, server, cb) => {
